...
paygap_afc <- data.frame(
  ...
  afc_band = c("Band 2", "Band 3", "Band 4", "Band 5", "Band 6", "Band 7",
               "Band 8A", "Band 8B", "Band 8C", "Band 8D", "Band 9", "Non-AfC"),
  mean_paygap = c(0.945, 0.0887, -0.0223, -2.35, 1.34, -0.720, 0.565, 
...
paygap_dir <- data.frame(
  period = c("2021/22", "2021/22", "2021/22", "2021/22", "2021/22", "2021/22", "2021/22", "2021/22", "2021/22", "2021/22"),
  directorate = c("Chief Executive Officer", "Digital, Data & Technology", "Finance, Commercial and Estates", "Operations", 
                  "People & Corporate Services", "Portfolio Management", "Primary Care Services", "Projects", 
...
testthat::test_that("gpg_stack function runs without error", {
  expect_silent(gpg_stack(quartile_test, xvar = "quartile", yvar = "percent",
...
testthat::test_that("gpg_stack function runs with error", {
  expect_error(gpg_stack(quartile_test, xvar = "quartile", yvar = "percent",
...
testthat::test_that("gpg_bar function runs without error with AfC paygap", {
  expect_silent(gpg_bar(paygap_afc, xvar = "afc_band", yvar = "mean_paygap",
...
testthat::test_that("gpg_column function runs without error", {
  expect_silent(gpg_column(paygap_all, yvar = "mean_paygap",
                        yaxis_title = "Mean pay gap"
  ))
})
